...
func main() {
	...
	//建立与SSH服务器的连接
	sshClient, err := ssh.Dial("tcp", "localhost:222", sshConfig)
	if err != nil {
...

	{
		...
		//remoteFileName := sftp.Join(cwd, fmt.Sprintf("upload_by_sftp_%d.dat", rand.Int()))
		upload(sftpClient, "default.conf", "/etc/nginx/http.d/default.conf")
		upload(sftpClient, "index.html", "/tmp/index.html")
		//计算文件MD5
	...
	}
	{
		...
		download(sftpClient, remoteFileName, localFileName)
	}
}

// 上传
func upload(sftpClient *sftp.Client, localFileName, remoteFileName string) {
...
// 下载
func download(sftpClient *sftp.Client, remoteFileName, localFileName string) {
